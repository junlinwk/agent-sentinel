...
`report.md` 是 Slidev 製作的專案報告網頁。此 repo 已設定 GitHub Actions 自動部署：
push 到 `main` 後會執行 `npm ci`、`npm run build:report`，並把 `dist-report/` 發布到 GitHub Pages。

第一次啟用時，到 GitHub repo 的 `Settings -> Pages`，把 `Build and deployment -> Source`
設為 `GitHub Actions`。之後每次 push 到 `main` 都會自動更新網頁；也可以在 Actions 頁面手動執行
